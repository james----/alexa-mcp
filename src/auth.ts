...
import type { AuthData } from './types/alexa.js';
...
const PROXY_PORT = (() => {
  ...
  if (raw === undefined || raw === '') return 3457;
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    console.error(
      `Invalid ALEXA_PROXY_PORT: "${raw}". Expected an integer between 1 and 65535.`
    );
    process.exit(1);
  }
  return port;
 ...
 */
function ensureSecureAuthDir(): void {
  mkdirSync(AUTH_DIR, { recursive: true, mode: 0o700 });
  try {
    if ((statSync(AUTH_DIR).mode & 0o077) !== 0) chmodSync(AUTH_DIR, 0o700);
  } catch {
 ...
 */
let saveSeq = 0;
function saveAuthFile(authData: AuthData): void {
  ensureSecureAuthDir();
...
function runProxyAuth(): void {
  ...
  try {
    const legacyStore = join(
      dirname(require.resolve('alexa-cookie2')),
      'lib',
  ...
  try {
    ...
    hpm.createProxyMiddleware = function (context: any, options: any) {
      if (options) {
        ...
        options.router = function (req: any) {
        ...
        options.onProxyReq = function (proxyReq: any, req: any, res: any) {
        ...
        options.onProxyRes = function (proxyRes: any, req: any, res: any) {
          ...
          if (proxyRes && proxyRes.headers && proxyRes.headers['set-cookie']) {
            proxyRes.headers['set-cookie'] = proxyRes.headers['set-cookie'].map((c: string) =>
              c.replace(/;\s*Domain=[^;]+/gi, '').replace(/;\s*Secure/gi, '')
            );
          }
          if (typeof origOnProxyRes === 'function') {
            return origOnProxyRes.apply(this, arguments as any);
          }
        };
      }
