...
export function registerTools(server, client) {
    ...
    server.registerTool('alexa_list_devices', {
        ...
        description: 'List all Amazon Echo devices in your account with their serial numbers, types, and online status.',
    ...
    // ============================================================
    server.registerTool('alexa_list_smarthome_devices', {
        title: 'List Smart Home Devices',
        description: 'List all smart home devices registered in Alexa (lights, switches, sensors, etc).',
        inputSchema: z.object({}),
    }, async () => {
        try {
    ...
    // ============================================================
    server.registerTool('alexa_list_groups', {
        title: 'List Smart Home Groups',
        description: 'List all smart home groups/rooms configured in Alexa. Shows which devices are assigned to each group.',
        inputSchema: z.object({}),
    }, async () => {
        try {
    ...
    server.registerTool('alexa_announce', {
        ...
        description: 'Send a voice announcement to a specific Echo device. The message will be spoken aloud.',
        inputSchema: z.object({
            serialNumber: z
                .string()
    ...
    // ============================================================
    server.registerTool('alexa_text_command', {
        title: 'Send Text Command to Alexa',
        description: 'Send a text command to Alexa as if you spoke it. Example: "turn on the kitchen lights"',
        inputSchema: z.object({
            serialNumber: z
                .string()
    ...
    // TOOL: Set Volume
    // ============================================================
    server.registerTool('alexa_set_volume', {
    ...
    // ============================================================
    server.registerTool('alexa_get_volumes', {
    ...
    server.registerTool('alexa_list_routines', {
        ...
        description: 'List all Alexa routines configured in the account. Shows routine names, triggers, and actions.',
        inputSchema: z.object({}),
    }, async () => {
        try {
    ...
    // ============================================================
    server.registerTool('alexa_execute_routine', {
        title: 'Execute Alexa Routine',
        description: 'Execute an existing Alexa routine by providing its automation definition (from alexa_list_routines).',
        inputSchema: z.object({
            routine: z
                .any()
                .describe('The full routine/automation object as returned by alexa_list_routines'),
        }),
    }, async ({ routine }) => {
        try {
    ...
    // ============================================================
    server.registerTool('alexa_list_lists', {
        title: 'List Alexa Lists',
        description: 'Get all Alexa lists (shopping lists, to-do lists, custom lists).',
        inputSchema: z.object({}),
    }, async () => {
        try {
    ...
    // TOOL: Get List Items
    // ============================================================
    server.registerTool('alexa_get_list_items', {
        title: 'Get List Items',
        description: 'Get all items from a specific Alexa list.',
        inputSchema: z.object({
            listId: z
                .string()
                .describe('The list ID (use alexa_list_lists to find it)'),
        }),
    }, async ({ listId }) => {
        try {
    ...
    // ============================================================
    server.registerTool('alexa_add_list_item', {
    ...
    // TOOL: Do Not Disturb
    // ============================================================
    server.registerTool('alexa_do_not_disturb', {
    ...
    // ============================================================
    server.registerTool('alexa_query_device', {
        title: 'Query Smart Home Device',
        description: 'Query the current state of one or more smart home devices (on/off, brightness, temperature, etc).',
        inputSchema: z.object({
            entityIds: z
                .array(z.string())
                .describe('Array of smart home entity IDs to query (from alexa_list_smarthome_devices)'),
        }),
    }, async ({ entityIds }) => {
        try {
    ...
    server.registerTool('alexa_speak', {
        title: 'Speak Text',
        description: 'Make Alexa speak plain text directly on a specific Echo device using native text-to-speech (no announcement chime).',
        inputSchema: z.object({
            serialNumber: z
                .string()
                .describe('Serial number of the Echo device (use alexa_list_devices to find it)'),
    ...
    // TOOL: Speak SSML
    // ============================================================
    server.registerTool('alexa_speak_ssml', {
        title: 'Speak SSML',
        description: 'Make Alexa speak using SSML (Speech Synthesis Markup Language) for advanced voice control.',
        inputSchema: z.object({
            serialNumber: z
                .string()
                .describe('Serial number of the Echo device'),
            ssml: z
                .string()
                .describe('SSML content (e.g., "<speak>Hello <break time=\\"1s\\"/> World</speak>")'),
        }),
    }, async ({ serialNumber, ssml }) => {
        try {
    ...
    // ============================================================
    server.registerTool('alexa_update_group', {
        title: 'Update Smart Home Group',
        description: 'Update an existing Alexa smart home group/room. Sets the name and full list of appliance IDs.',
        inputSchema: z.object({
            groupId: z
                .string()
    ...
    // ============================================================
    server.registerTool('alexa_create_group', {
        title: 'Create Smart Home Group',
        description: 'Create a new Alexa smart home group/room with a name and optional list of appliance IDs.',
        inputSchema: z.object({
            name: z.string().describe('The group/room name'),
            applianceIds: z
                .array(z.string())
    ...
    // ============================================================
    server.registerTool('alexa_delete_group', {
    ...
    // ============================================================
    server.registerTool('alexa_smarthome_action', {
        title: 'Execute Smart Home Device Action',
        description: 'Control Alexa smart home devices directly (turn on/off, set brightness, set temperature, lock/unlock).',
        inputSchema: z.object({
            entityId: z
                .string()
            ...
            value: z
                ...
                .optional()
                .describe('Value for the action: brightness (0-100), temperature in degrees, lock state ("LOCKED"/"UNLOCKED"), color Kelvin or name'),
            entityType: z
    ...
    }, async ({ entityId, action, value, entityType, }) => {
        try {
            let params = { action };
            ...
            const res = await client.executeSmarthomeDeviceAction(entityId, params, entityType || 'APPLIANCE');
            return {
                content: [
                    {
                        type: 'text',
    ...
    // ============================================================
    server.registerTool('alexa_play_music', {
        title: 'Play Music via Streaming Provider',
        description: 'Search and stream music on an Echo device from Amazon Music, Spotify, Apple Music, TuneIn, iHeartRadio, or Deezer.',
        inputSchema: z.object({
            serialNumber: z
                .string()
                .describe('Serial number of the Echo device (use alexa_list_devices to find it)'),
    ...
    }, async ({ serialNumber, searchPhrase, provider, }) => {
        try {
    ...
    // ============================================================
    server.registerTool('alexa_play_audible', {
    ...
    }, async ({ serialNumber, searchPhrase, }) => {
        try {
    ...
    server.registerTool('alexa_media_control', {
        ...
        description: 'Control media playback (stop, pause, play, next, previous) on a specific Echo or all devices.',
        inputSchema: z.object({
            serialNumber: z
                .string()
                .describe('Serial number of the Echo device (use alexa_list_devices to find it), or "all" to stop all'),
            action: z
    ...
    // ============================================================
    server.registerTool('alexa_set_equalizer', {
        title: 'Set Device Equalizer',
        description: 'Adjust bass, midrange, and treble levels on an Echo speaker (-6 to +6 dB).',
        inputSchema: z.object({
            serialNumber: z
                .string()
                .describe('Serial number of the Echo device (use alexa_list_devices to find it)'),
    ...
    }, async ({ serialNumber, bass, midrange, treble, }) => {
        try {
    ...
    // ============================================================
    server.registerTool('alexa_set_reminder', {
        ...
        inputSchema: z.object({
            ...
            timestamp: z
                .union([z.string(), z.number()])
                .describe('Target time as ISO 8601 string (e.g. "2026-09-12T18:00:00") or epoch milliseconds'),
        }),
    }, async ({ serialNumber, text, timestamp, }) => {
        try {
    ...
    // ============================================================
    server.registerTool('alexa_get_notifications', {
        title: 'Get Alarms, Timers & Reminders',
        description: 'List active alarms, timers, and reminders across all Echo devices in the account.',
        inputSchema: z.object({
            type: z
    ...
    // ============================================================
    server.registerTool('alexa_set_alarm_volume', {
        title: 'Set Alarm & Notification Volume',
        description: 'Adjust alarm and notification volume level independently from media/music volume.',
        inputSchema: z.object({
            serialNumber: z
                .string()
                .describe('Serial number of the Echo device (use alexa_list_devices to find it)'),
    ...
    // ============================================================
    server.registerTool('alexa_play_sound', {
        title: 'Play Built-in Sound Effect',
        description: 'Play a built-in sound effect or chime on an Echo device (e.g. bells, doorbells, boings, applause, buzzers).',
        inputSchema: z.object({
            serialNumber: z
                .string()
                .describe('Serial number of the Echo device (use alexa_list_devices to find it)'),
            soundId: z
                .string()
                .describe('Sound identifier (e.g. "amzn_sfx_doorbell_chime_01", "bell_01", "boing_01", "applause_01", "buzzer_01")'),
        }),
    }, async ({ serialNumber, soundId }) => {
        try {
    ...
    // ============================================================
    server.registerTool('alexa_curated_tts', {
        title: 'Speak Curated Phrase',
        description: 'Make Alexa speak a randomized built-in phrase from a curated category.',
        inputSchema: z.object({
            serialNumber: z
                .string()
                .describe('Serial number of the Echo device (use alexa_list_devices to find it)'),
    ...
    // ============================================================
    server.registerTool('alexa_play_behavior', {
        title: 'Play Native Routine Behavior',
        description: 'Trigger native routine behaviors like weather report, traffic update, flash briefing, jokes, fun facts, or stories.',
        inputSchema: z.object({
            serialNumber: z
                .string()
                .describe('Serial number of the Echo device (use alexa_list_devices to find it)'),
    ...
    // ============================================================
    server.registerTool('alexa_fire_tv_control', {
        title: 'Control Fire TV Device',
        description: 'Control a Fire TV or Fire TV Cube (turnOn, turnOff, pause, resume, navigateHome).',
        inputSchema: z.object({
            serialNumber: z
                .string()
                .describe('Serial number of the Fire TV device (use alexa_list_devices to find it)'),
            command: z
    ...
    // ============================================================
    server.registerTool('alexa_get_history', {
        title: 'Get Alexa Voice History',
        description: 'Retrieve recent voice interaction history (what users said to Alexa, device used, timestamps).',
        inputSchema: z.object({
            limit: z
                .number()
                .optional()
