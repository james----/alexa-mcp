...
import { AlexaHttp2Push } from './alexa-http2push.js';
...
class AlexaRemote extends EventEmitter {
    ...
    authApiBearerExpiry;
    activityCsrfToken;
    activityCsrfTokenExpiry;
    activityCsrfTokenReferer;
    lastVolumes;
    lastEqualizer;
    lastPushedActivity;
    activityUpdateQueue;
    activityUpdateNotFoundCounter;
    _options;
    constructor() {
        super();
        this.serialNumbers = {};
        ...
        this.endpoints = null;
        this.baseUrl = 'alexa.amazon.com';
        this._locale = 'en-US';
        this._options = { amazonPage: 'amazon.com' };
    ...
    init(cookie, callback) {
        if (typeof cookie === 'object') {
            ...
            if (this._options.apiUserAgentPostfix === undefined) {
                this._options.apiUserAgentPostfix = 'AlexaRemote/8.0.4';
            }
            this._options.amazonPage = this._options.amazonPage || 'amazon.com';
    ...
    prepare(callback) {
        this.getAccount((err, result) => {
            ...
            this.initDeviceState(() => this.initWakewords(() => this.initBluetoothState(() => this.initNotifications(callback))));
        });
        return this;
    }
                ...
                this.getDevicePreferences((err, devicePrefs) => {
                    ...
                    joinedDevices.forEach((device) => {
                        ...
                        if (!existingDevice) {
                            ...
                            if (device.parentDeviceSerialNumber && processedSerialNumbers.includes(device.serialNumber))
                                return;
                        ...
                        device.isControllable = (device.capabilities.includes('AUDIO_PLAYER') ||
                        ...
                        device.hasMusicPlayer = (device.capabilities.includes('AUDIO_PLAYER') ||
                            device.capabilities.includes('AMAZON_MUSIC') ||
                            device.deviceFamily === 'FIRE_TV');
    ...
    simulateActivity(deviceSerialNumber, destinationUserId) {
        if (!this._options.autoQueryActivityOnTrigger)
            return;
        if (this.activityUpdateTimeout && this.activityUpdateQueue.some(entry => entry.deviceSerialNumber === deviceSerialNumber && entry.destinationUserId === destinationUserId))
            return;
        ...
        this.alexahttp2Push.on('command', (command, payload) => {
            ...
            switch (command) {
                ...
                case 'PUSH_EQUALIZER_STATE_CHANGE':
                    ...
                    */
                    if (!this.lastEqualizer[payload.dopplerId.deviceSerialNumber] ||
        ...
        this.getCustomerHistoryRecords({
            ...
            if (!err) {
                    ...
                    if (entry.key) { // deprecated
                        ...
                        if (found.length === 0) {
                            ...
                            found.forEach((activity, index) => {
                                if (activity.data.utteranceType === 'WAKE_WORD_ONLY' && index === 0 && this.activityUpdateNotFoundCounter > 0 && found.length > 1)
                                    return;
                ...
                if (lastFoundQueueIndex < 0) {
                    ...
                    if ((lastFoundQueueIndex === -1 && this.activityUpdateNotFoundCounter > 2) || // 2 tries without wakeword
    ...
    // API Host call without Authorization header
    httpsApiGet(path, callback, flags) {
        if (!this.endpoints || !this.endpoints.alexaApiUrl) {
            this._options.logger && this._options.logger(`Alexa-Remote: No endpoint set for alexaApiUrl: ${JSON.stringify(this.endpoints)}`);
            return callback && callback(new Error(`No endpoint set for alexaApiUrl: ${JSON.stringify(this.endpoints)}`));
        }
    ...
    httpsGetCall(path, callback, flags = {}) {
        ...
        const options = {
            ...
            headers: {
                ...
                'Accept-Language': this._options.acceptLanguage || 'en-US',
    ...
    getUsersMe(callback) {
        this.httpsGetCall('/api/users/me?platform=ios&version=2.2.651540.0', callback);
    }
    getHousehold(callback) {
        this.httpsGet('/api/household', callback);
    }
    getMedia(serialOrName, callback) {
        const dev = this.find(serialOrName);
        if (!dev)
            return callback && callback(new Error('Unknown Device or Serial number'), null);
    ...
    getLists(callback) {
        this.getListsV2((err, lists) => {
            if (err) {
                this._options.logger && this._options.logger(`Alexa-Remote: Error getting lists: ${err.message}`);
                return callback && callback(err);
            }
            const mappedList = lists.map(list => {
                let count = null;
                try {
                    count = JSON.parse(list.totalActiveItemsCount).count;
                }
                catch (e) {
                    // Ignore
                }
                return {
                    id: Array.isArray(list.id) && list.id.length > 0 ? list.id[0] : list.listId,
                    listId: list.listId,
                    itemId: list.listId,
                    name: list.listName || list.listType,
                    createdDateTime: list.createAt,
                    updatedDateTime: list.updateAt,
                    totalActiveItemsCount: count,
                    ownerCustomerId: list.customerId,
                };
            });
            callback && callback(null, mappedList);
        });
    }
    getListsV2(callback) {
        // request options
        const request = {
            ...
            'data': JSON.stringify({
                listAttributesToAggregate: [
                    ...
                    { type:'shareableInfo','marketplaceId':'A1PA6795UKMFR9','clientId':'AlexaApp' },
        ...
        this.httpsGet(`https://www.${this._options.amazonPage}/alexashoppinglists/api/v2/lists/fetch`, (err, res) => callback && callback(err, res && res.listInfoList), request);
    }
    /** @deprecated Use getListV2 instead */
    getList(listId, callback) {
        this.getListV2(listId, (err, list) => {
            if (err) {
                this._options.logger && this._options.logger(`Alexa-Remote: Error getting lists: ${err.message}`);
                return callback && callback(err);
            }
    ...
    getListV2(listId, callback, limit) {
        // request options
        const request = {
            'method': 'POST',
            'data': JSON.stringify({
                listAttributesToAggregate: [
                    /*{'type':'listCategories'},
                    {'type':'shareableInfo','marketplaceId':'A1PA6795UKMFR9','clientId':'AlexaApp'},
                    {'type':'keywordRecommendations','clientId':'AlexaApp','marketplaceId':'A1PA6795UKMFR9','locale':'de_DE'},*/
                    { 'type': 'totalActiveItemsCount' },
                    /*{'type':'categoryOverride'}*/
                ]
            })
        };
        this.httpsGet(`https://www.${this._options.amazonPage}/alexashoppinglists/api/v2/lists/${listId}/items/fetch?limit=${limit || 100}`, (err, res) => callback && callback(err, res && res.listInfo), request);
    }
    /**
    ...
    getListItems(listId, options, callback) {
        ...
        this.getListItemsV2(listId, options, (err, items) => {
            if (err) {
                this._options.logger && this._options.logger(`Alexa-Remote: Error getting list: ${err.message}`);
                return callback && callback(err);
            }
            ...
            callback && callback(null, mappedItems);
        });
    }
    /**
     ...
     *
     */
    getListItemsV2(listId, options, callback) {
        if (typeof options === 'function') {
            callback = options;
            options = {};
        }
        if (!listId) {
            this._options.logger && this._options.logger('Alexa-Remote: Error getting list items: No listId provided');
            return callback && callback(new Error('No listId provided'));
        }
        const limit = options.limit || 100;
        const request = {
            'method': 'POST',
            'data': JSON.stringify({
                itemAttributesToProject: ['quantity', /*'speakerId','asinInfo',*/ 'note', /*'dealInfo'*/],
                /*itemAttributesToAggregate:[
                    {type:'categorization'},{type:'speakerInfo','marketplaceId':'A1PA6795UKMFR9'}
                ],*/
                /*listAttributesToAggregate:[
                    {type:'listCategories'},
                    {type:'shareableInfo','marketplaceId':'A1PA6795UKMFR9','clientId':'AlexaApp'},
                    {type:'keywordRecommendations','clientId':'AlexaApp','marketplaceId':'A1PA6795UKMFR9','locale':'de_DE'},
                    {type:'totalActiveItemsCount'},{type:'categoryOverride'}
                ]*/
            })
        };
        // send request
        this.httpsGet(`https://www.${this._options.amazonPage}/alexashoppinglists/api/v2/lists/${listId}/items/fetch?limit=${limit}`, (err, res) => callback && callback(err, res && res.itemInfoList), request);
    }
    addListItem(listId, options, callback) {
        ...
        const request = {
            ...
            'data': JSON.stringify({
                items: [{
                        itemType: 'KEYWORD',
                        itemName: options.value,
                        /*itemAttributesToCreate: [{ type : 'speakerId'}],
                        itemAttributesToAggregate : [
                            { type:'categorization'},
                            { type:'speakerInfo', marketplaceId: 'A1PA6795UKMFR9'}
                        ]*/
                    }]
            })
        };
        // send request
        this.httpsGet(`https://www.${this._options.amazonPage}/alexashoppinglists/api/v2/lists/${listId}/items`, (err, res) => callback && callback(err, res && res.itemInfoList), request);
    }
    updateListItem(listId, listItemId, options, callback) {
        ...
        const itemAttributesToUpdate = [];
        const itemAttributesToRemove = [];
        for (const key in options) {
            switch (key) {
                ...
                case 'note':
                    ...
                    break;
                case 'completed':
                    itemAttributesToUpdate.push({ type: 'itemStatus', value: options.completed ? 'COMPLETE' : 'ACTIVE' });
                    break;
                case 'itemStatus':
                    if (options.completed !== undefined) {
                        break; // ignore, because itemStatus is handled by completed
                    }
        ...
        // send request
        this.httpsGet(`https://www.${this._options.amazonPage}/alexashoppinglists/api/v2/lists/${listId}/items/${listItemId}?version =${options.version}`, (err, res) => callback && callback(err, res && res.itemInfoList), request);
    }
    deleteListItem(listId, listItemId, options, callback) {
        // providing a version is mandatory
        if (typeof options !== 'object' || !options.version) {
            const errors = [];
            if (!options.version && callback) {
                errors.push('Providing the current version via options is mandatory!');
            }
        ...
        // send request
        this.httpsGet(`https://www.${this._options.amazonPage}/alexashoppinglists/api/v2/lists/${listId}/items/${listItemId}?version=${options.version}`, callback, request);
    }
    ...
    createNotificationObject(serialOrName, type, label, value, status, sound, recurring) {
    ...
    _getCustomerHistoryRecords(options, callback) {
        let url = `https://www.${this._options.amazonPage}/alexa-privacy/apd/rvh/customer-history-records` +
            `?startTime=${options.startTime || 0}` +
            `&endTime=${options.endTime || 2147483647000}` +
            `&recordType=${options.recordType || 'VOICE_HISTORY'}` +
            `&maxRecordSize=${options.maxRecordSize || 20}`;
        //ignoring maxSize for now and just take the 10 that are returned. `&maxRecordSize=${options.maxRecordSize || 1}`,
        this.httpsGet(url, (err, result) => {
            if (err || !result)
    ...
    getContacts(options, callback) {
        ...
        if (options.homeGroupId === undefined)
            options.homeGroupId = '';
        this.httpsGet(`https://alexa-comms-mobile-service.${this._options.amazonPage}/users/${this.commsId}/contacts
            ...
            &homeGroupId=${options.homeGroupId}`, function (err, result) {
            callback(err, result);
        });
    }
    getConversations(options, callback) {
        ...
        this.httpsGet(`https://alexa-comms-mobile-service.${this._options.amazonPage}/users/${this.commsId}/conversations
            ?latest=${options.latest}
            &includeHomegroup=${options.includeHomegroup}
            &unread=${options.unread}
            &modifiedSinceDate=${options.modifiedSinceDate}
            &includeUserName=${options.includeUserName}`, function (err, result) {
            callback(err, result);
        });
    }
    ...
    sendMessage(serialOrName, command, value, callback) {
        ...
        this.httpsGet(`/api/np/command?deviceSerialNumber=${dev.serialNumber}&deviceType=${dev.deviceType}`, callback, {
            method: 'POST',
    ...
    createSequenceNode(command, value, serialOrName, overrideCustomerId) {
        ...
        if (serialOrName && !Array.isArray(serialOrName)) {
            currDevice = this.find(serialOrName);
            if (currDevice) {
        ...
        else if (serialOrName && Array.isArray(serialOrName) && serialOrName.length > 0) {
            currDevice = this.find(serialOrName[0]);
            if (currDevice) {
                if (currDevice.preferences && currDevice.preferences.timeZoneId !== undefined) {
                    deviceTimezoneId = currDevice.preferences.timeZoneId;
                }
        ...
        switch (command) {
            ...
            case 'notification': {
                ...
                let title = this._options.notificationTitle || 'Alexa';
            ...
            case 'ssml':
                ...
                seqNode.operationPayload.content = [
                    {
                        'locale': (currDevice && currDevice.preferences && currDevice.preferences.locale) || this._options.acceptLanguage || 'en-US',
                        'display': {
                            'title': this._options.announcementTitle || 'Alexa',
    ...
    sendSequenceCommand(serialOrName, command, value, overrideCustomerId, callback) {
        ...
        const currentLocale = (dev && dev.preferences && dev.preferences.locale) || this._options.acceptLanguage || 'en-US';
        reqObj.sequenceJson = reqObj.sequenceJson.replace(/"locale":"ALEXA_CURRENT_LOCALE"/g, `"locale":"${currentLocale}"`);
        reqObj.sequenceJson = reqObj.sequenceJson.replace(/"locale":"de-DE"/g, `"locale":"${currentLocale}"`);
    ...
    getMusicProviders(callback) {
        this.httpsGet('/api/behaviors/entities?skillId=amzn1.ask.1p.music', callback, {
            headers: {
                'Routines-Version': '3.0.264101'
            }
        });
    }
    playMusicProvider(serialOrName, providerId, searchPhrase, callback) {
        ...
        const devLocale = (dev && dev.preferences && dev.preferences.locale) || this._options.acceptLanguage || 'en-US';
        ...
        this.httpsGet(`/api/behaviors/operation/validate`, (err, res) => {
            if (err) {
    ...
    playAudible(serialOrName, searchPhrase, callback) {
        ...
        if (searchPhrase === '')
            return callback && callback(new Error('Searchphrase empty'), null);
        const devLocale = (dev && dev.preferences && dev.preferences.locale) || this._options.acceptLanguage || 'en-US';
        const shortLocale = devLocale.split('-')[0] || 'en';
        const operationPayload = {
            'deviceType': dev.deviceType,
            'deviceSerialNumber': dev.serialNumber,
            'locale': shortLocale,
    ...
    sendTextMessage(conversationId, text, callback) {
        ...
        this.httpsGet(`https://alexa-comms-mobile-service.${this._options.amazonPage}/users/${this.commsId}/conversations/${conversationId}/messages`, callback, {
            method: 'POST',
        ...
        this.httpsGet('/api/device-preferences?cached=true', (err, res) => {
            if (serialOrName) {
                ...
                if (!err && res && res.devicePreferences && Array.isArray(res.devicePreferences)) {
                    const devicePreferences = res.devicePreferences.filter(pref => pref.deviceSerialNumber === device.serialNumber);
                    if (devicePreferences.length > 0) {
                        return callback && callback(null, devicePreferences[0]);
                    }
                    else {
                        return callback && callback(new Error(`No Device Preferences found for ${device.serialNumber}`), null);
                    }
                }
            }
            callback && callback(err, res);
    ...
    getDeviceWifiDetails(serialOrName, callback) {
        ...
        this.httpsGet(`/api/device-wifi-details?deviceSerialNumber=${dev.serialNumber}&deviceType=${dev.deviceType}`, callback);
    }
    ...
    getSmarthomeDevices(callback) {
        this.getSmarthomeDevicesV2((err, devices) => {
            if (err) {
                return callback && callback(err);
            }
            const mappedResults = devices.map(device => ({
                ...device,
                ...device.legacyAppliance,
                ...device.legacyIdentifiers,
                legacyAppliance: undefined,
                legacyIdentifiers: undefined,
            }));
            const amazonBridgeDetails = {};
            mappedResults.forEach(device => {
                const id = device.applianceId;
                if (!id) {
                    this._options.logger && this._options.logger(`Ignoring device without applianceId: ${JSON.stringify(device)}`);
                    return;
                }
                if (device &&
                    device.dmsIdentifier &&
                    device.dmsIdentifier.deviceType &&
                    device.dmsIdentifier.deviceType.value &&
                    (device.dmsIdentifier.deviceType.value.text === 'A2IVLV5VM2W81' ||
                        device.dmsIdentifier.deviceType.value.text === 'A2TF17PFR55MTB')) {
                    return; // Ignore Alexa App devices
                }
                amazonBridgeDetails[`LambdaBridge_${id}`] = amazonBridgeDetails[`LambdaBridge_${id}`] || {
                    applianceDetails: {
                        applianceDetails: {}
                    }
                };
                amazonBridgeDetails[`LambdaBridge_${id}`].applianceDetails.applianceDetails[id] = device;
            });
            const result = {
                locationDetails: {
                    Default_Location: {
    ...
    getSmarthomeDevicesV2(callback) {
        const request = {
            method: 'POST',
            data: JSON.stringify({ query: SmartHomeEndpointsGraphQlQuery }),
            timeout: 30000
        };
        this.httpsGet('/nexus/v1/graphql', function (err, res) {
            if (!err && (!res.data || !res.data.endpoints || !Array.isArray(res.data.endpoints.items))) {
                err = new Error('Invalid response from server');
            }
            if (err) {
                return callback && callback(err, res);
            }
            callback && callback(null, res.data.endpoints.items);
        }, request);
    }
    ...
    getSmarthomeEntities(callback) {
        this.httpsGet('/api/behaviors/entities?skillId=amzn1.ask.1p.smarthome', callback, {
            headers: {
                'Routines-Version': '3.0.264101'
            },
            timeout: 30000
    ...
    getFireTVEntities(callback) {
        this.httpsGet('/api/behaviors/entities?skillId=amzn1.ask.1p.routines.firetv', callback, {
            headers: {
                'Routines-Version': '3.0.264101'
            },
            timeout: 30000
    ...
    getSmarthomeBehaviourActionDefinitions(callback) {
        this.httpsGet('/api/behaviors/actionDefinitions?skillId=amzn1.ask.1p.smarthome', callback, {
            headers: {
                'Routines-Version': '3.0.264101'
            },
            timeout: 30000
    ...
    getRoutineSoundList(callback) {
        this.httpsGet('/api/behaviors/entities?skillId=amzn1.ask.1p.sound', callback, {
            headers: {
                'Routines-Version': '3.0.264101'
            },
            timeout: 30000
    ...
    setEnablementForSmarthomeDevice(smarthomeDevice, enabled, callback) {
        ...
        this.httpsGet(`/api/phoenix/v2/appliance/${smarthomeDevice}/enablement`, callback, flags);
    }
...
//# sourceMappingURL=alexa-remote.js.map